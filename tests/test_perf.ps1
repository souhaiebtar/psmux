...
$port = (Get-Content "$userHome\.psmux\$SESSION_NAME.port" -ErrorAction SilentlyContinue).Trim()
$key = (Get-Content "$userHome\.psmux\$SESSION_NAME.key" -ErrorAction SilentlyContinue).Trim()
...
for ($c = 0; $c -lt $cycles; $c++) {
    ...
    # Wait for port file
    $portPath = "$userHome\.psmux\perf_cycle_$c.port"
    $maxWait = 30
