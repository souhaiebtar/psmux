...
use crate::pane::{
    create_window, create_window_raw, kill_active_pane, spawn_warm_pane, split_active_with_command,
};
use crate::platform::install_console_ctrl_handler;
use crate::tree::{
    self, active_pane, active_pane_mut, find_window_index_by_id, focus_pane_by_id,
    focus_pane_by_index, get_active_pane_id, kill_all_children, path_exists, resize_all_panes,
};
use crate::types::{
...
use crate::commands::{format_action, parse_command_to_action, parse_menu_definition};
use crate::config::{
    format_key_binding, load_config, normalize_key_for_binding, parse_config_content,
    parse_config_line, parse_key_string,
};
use crate::copy_mode::{
...
use crate::help;
use crate::input::{move_focus, send_key_to_active, send_paste_to_active, send_text_to_active};
use crate::layout::{
...
pub fn run_server(
    ...
    let _ = std::fs::write(
        format!("{}\\.psmux\\debug_server.log", home_dbg),
        format!("run_server called for '{}'\n", session_name),
    );
    ...
    if let Some(n) = window_name {
        app.windows.last_mut().map(|w| w.name = n);
    }
                    ...
                    match req {
                        ...
                        CtrlReq::ClientDetach(cid) => {
                            ...
                            if app.attached_clients == 0 && app.destroy_unattached {
                                ...
                                let has_live_pane = app
                                    .windows
                                    .iter()
                                    .any(|w| crate::tree::count_panes(&w.root) > 0);
                                if !has_live_pane {
                        ...
                        CtrlReq::SetOptionAppend(option, value) => {
                            ...
                            if option.starts_with('@') {
                                let existing =
                                    app.environment.get(&option).cloned().unwrap_or_default();
                                app.environment
