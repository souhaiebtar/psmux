    ...
    loop {
        ...
        if let Some(rx) = app.control_rx.as_ref() {
            if let Ok(req) = rx.recv_timeout(Duration::from_millis(timeout_ms)) {
                ...
                CtrlReq::MouseMove(x,y) => { { let p = format!("{}/.psmux/mouse_debug.log", std::env::var("USERPROFILE").unwrap_or_default()); let _ = std::fs::OpenOptions::new().create(true).append(true).open(&p).map(|mut f| { use std::io::Write; let _ = writeln!(f, "[SERVER] MouseMove({},{}) mouse_enabled={}", x, y, app.mouse_enabled); }); } if app.mouse_enabled { remote_mouse_motion(&mut app, x, y); state_dirty = true; echo_pending_until = Some(Instant::now()); } }
